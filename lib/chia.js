...
class Chia {
  ...
  get chiaCertificateDirectory() {
    return join(this.rootDirectory, 'config', 'ssl');
  }
  ...
  async createPlot({
    ...
    if (poolContractAddress) {
      args.push('-c', poolContractAddress);
    }
    if (!useBitfield) {
      args.push('-e');
    }
  ...
  _spawnBinary(args) {
    return spawn(`./${basename(this.binaryPath)}`, args, {
      ...
      stdio: 'pipe',
      env: { ...process.env, CHIA_ROOT: this.rootDirectory },
    });
  }

  get _chiaConfigPath() {
    return join(this.rootDirectory, 'config', 'config.yaml');
  }
